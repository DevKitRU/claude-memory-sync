...
.DESCRIPTION
    ...
    4. Task Scheduler auto-pull работает
...
# 1. Junction
Write-Info "1) Junction"
if (-not $ClaudeMemory) {
    Write-Err "   Не нашёл Junction в %USERPROFILE%\.claude\projects\*\memory указывающий на $GitMemory"
    Write-Err "   Запусти .\setup\windows.ps1"
    $Issues++
} else {
    Write-Ok "   Junction корректный: $ClaudeMemory -> $GitMemory"
}
...
if (-not $task) {
    ...
    if ($null -eq $lastRun -or $lastRun.Year -lt 2000) {
        Write-WarnMsg "   Задача есть, но ещё ни разу не запускалась (или только что создана)"
    } else {
        ...
        if ($taskInfo.LastTaskResult -ne 0) {
            Write-WarnMsg "   Последний результат: $($taskInfo.LastTaskResult) (не 0)"
        }
    }
}
...
# 5. Файлы
Write-Info "5) Файлы"
$filesInGit = (Get-ChildItem $GitMemory -File -ErrorAction SilentlyContinue).Count
if ($ClaudeMemory) {
    ...
    Write-WarnMsg "   Junction не найден — пропускаю"
}
