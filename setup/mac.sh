...
# ——— Шаг 6: LaunchAgent для smart auto-pull (идемпотентно)
info "Шаг 6/7: smart auto-pull (LaunchAgent)"
...
cat > "$WRAPPER" <<EOF
#!/usr/bin/env bash
# Auto-generated by claude-memory-sync/setup/mac.sh. Не редактируй вручную.
set -u
LOG_FILE="$LOG_FILE"
REPO_DIR="$REPO_DIR"
GIT_BIN="$GIT_BIN"
...
cd "\$REPO_DIR" || { log "SKIP repo missing: \$REPO_DIR"; exit 0; }

if pgrep -x git >/dev/null 2>&1; then
    log "SKIP another git process is running"
    exit 0
fi

if [[ -n "\$("\$GIT_BIN" status --porcelain)" ]]; then
    log "SKIP local changes present; manual save/pull needed"
    exit 0
fi

"\$GIT_BIN" fetch --quiet --prune || { log "ERROR git fetch failed"; exit 0; }
counts="\$("\$GIT_BIN" rev-list --left-right --count '@{u}...HEAD' 2>/dev/null)" || {
    log "ERROR rev-list failed"
    exit 0
}
behind="\${counts%%[[:space:]]*}"
ahead="\${counts##*[[:space:]]}"
...
if [[ "\$behind" == "0" ]]; then
    log "OK up-to-date ahead=\$ahead"
    exit 0
fi

if [[ "\$ahead" != "0" ]]; then
    log "SKIP local commits ahead=\$ahead behind=\$behind; manual rebase needed"
    exit 0
fi

if "\$GIT_BIN" pull --ff-only --quiet; then
    log "OK pulled behind=\$behind"
else
...
echo "  • LaunchAgent запускает smart auto-pull раз в 15 мин"
